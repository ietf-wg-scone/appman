...
that policy. As part of 3rd option, Network element vendors and operators can identify genuine flows by using existing mechanism such as checking Server Name Indication to verify that flow is generated by genuine application endpoint. This would avoid the need of Network element to update the rate signal field in SCONE packets for flows which are not valid.
